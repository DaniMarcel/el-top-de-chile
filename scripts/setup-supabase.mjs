/**
 * Aplica el schema de Supabase y siembra las categorías + tiendas demo.
 * Uso: node scripts/setup-supabase.mjs
 */
import { readFileSync } from 'node:fs';
import pg from 'pg';

// Password real del usuario: 0x121212FFF.//.  (codificado: %2F por cada /)
// NOTA: el "." final y los "/" van codificados; el @ del password no existe.
// Si el password crudo es 0x121212FFF.//. entonces codificado es 0x121212FFF.%2F%2F.
const DATABASE_URL =
  'postgresql://postgres:0x121212FFF.%2F%2F.@db.exogasxtkdhfampoueef.supabase.co:5432/postgres';
...
try {
  ...
  // 2) Tiendas demo (mismo seed que local pero vía SQL directo)
  ...
  for (const [s, prev, amt] of txs) {
    await pool.query(
      ...
      [await sid(s), await (async (slug) => (await pool.query('SELECT category_id FROM stores WHERE slug = $1', [slug])).rows[0].category_id)(s), amt, 'TX-DEMO-' + s, prev ? await sid(prev) : null]
    );
  }
