...
export default async function StorePage({ params }: Props) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8 pt-10">
      <Link href={`/categoria/${store.cat_slug}`} className="text-sm font-medium text-accent hover:underline">
        ← Top {store.cat_name}
      </Link>
      ...
      <div className="card overflow-hidden">
        <div
          className={
            ...
            (isKing ? 'bg-gold-bg text-gold' : 'bg-card2 text-mut')
          }
        >
          {isKing
            ? '👑 Rey actual de ' + store.cat_name
            : store.position
            ? '#' + store.position + ' en ' + store.cat_name
            : 'Fuera del ranking'}
        </div>
        <div className="flex flex-col gap-6 p-6 sm:flex-row sm:items-start">
          <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl border border-line bg-card2 text-4xl">
            {store.logo_url ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={store.logo_url} alt={store.name} className="h-16 w-16 rounded-xl object-contain" />
            ) : (
              <span className="font-bold text-mut">{store.name.charAt(0).toUpperCase()}</span>
            )}
          </div>
          <div className="min-w-0 flex-1 space-y-3">
            ...
            <p className="text-sm text-mut">
              Pagó <b className="text-ink">{clp(store.current_price ?? 0)}</b> por el trono
              {store.claimed_at && (
                <>
                  {' '}
                  <span className="text-mut/60">({timeAgo(store.claimed_at)})</span>
                </>
              )}
            </p>
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              >
                Destronar a {store.name.split(' ')[0]} · {clp(required)}
              </Link>
            </div>
          </div>
        </div>
      ...
      <section className="space-y-4">
        <h2 className="text-xs font-bold uppercase tracking-wider text-mut">
      ...
      <section className="card bg-card2 p-6 text-center">
        <p className="text-mut">
          ¿Quieres que tu tienda aparezca acá arriba? El trono de {store.cat_name} hoy vale{' '}
          <b className="text-ink">{clp(top?.current_price ?? 0)}</b>.
        </p>
        <Link href="/reclamar" className="btn-primary mt-4">
          Reclamar el Top 1
        </Link>
      </section>
    </div>
